...
# Introduction: add one tiny change
...
To put our raster data in context, let's plot the Great Barrier Reef Marine Park boundary (dowloaded from [here](https://geohub-gbrmpa.hub.arcgis.com/datasets/bac38dff14ae4ff9a1c9de5d234e26f8_30/explore)). This is vector data. You might be used to handling vector data with the `sf` package, but `terra` can also be used for vector data manipulation. We can load a vector using `vect()`
...
#as an alternative, we can use the EPSG that we went to project into like this:
...
If you want to know why the projected rasters don't align with the projected vector grid see [this Q&A on Stackoverflow](https://stackoverflow.com/questions/78440026/how-does-the-r-package-terra-decide-on-the-template-raster-properties-when-pro)
